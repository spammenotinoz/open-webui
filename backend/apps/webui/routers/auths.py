...
from fastapi import Request, UploadFile, File, Depends, HTTPException, status, APIRouter
...
from apps.webui.models.auths import (
    SigninForm, SignupForm, AddUserForm, UpdateProfileForm, UpdatePasswordForm, UserResponse, SigninResponse, Auths, ApiKey
)
...
from config import (
    WEBUI_AUTH, WEBUI_AUTH_TRUSTED_EMAIL_HEADER, WEBUI_AUTH_TRUSTED_NAME_HEADER
)
...
async def update_profile(form_data: UpdateProfileForm, session_user=Depends(get_current_user)):
    if session_user:
...
async def signup(request: Request, response: Response, form_data: SignupForm):
    ...
    try:
        role = (
            "admin" if Users.get_num_users() == 0 else request.app.state.config.DEFAULT_USER_ROLE
        )
